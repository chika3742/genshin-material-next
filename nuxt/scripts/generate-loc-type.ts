...
export const generateLocType = async() => {
  const outputDir = "./types/generated"

  const locJa = parse((await fs.readFile(path.resolve("./locales/ja.yaml"))).toString()) as LocObject
  const locEn = parse((await fs.readFile(path.resolve("./locales/en.yaml"))).toString()) as LocObject
  ...
  await fs.writeFile(path.join(outputDir, "loc.g.ts"), locTs)
